...
parameters = struct(...
    ...
    'toggle_record_success',        true,...% default true
    ...
    'boxcar_refractory_period',     16,... %when set to 1 there is no refractory period
    'boxcar_window',                4,... %excitation boxcar
    'boxcar_scales',                [.25,.25,.25,.25],... % should sum to 1
    'boxcar_window_inh',            4,...
    'boxcar_scales_inh',            [.25,.25,.25,.25],...
    ...
    ... % random number seeds
    ...
    'connectivity',                 0.08,... %fan in is exact i.e. each postsyn. nrn is enervated by same number of nrns
    'desired_mean_z',               0.1,...
    ...
    ... % input settings
...
%     parameters.n_fanin = uint16(n_nrns*parameters.connectivity);
    ...
    parameters.nrn_viewing_range = n_nrns;
